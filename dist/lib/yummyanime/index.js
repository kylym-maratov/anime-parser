...
class YummyAnimeParser extends data_parser_1.default {
    constructor() {
        super(
            config_1.default.host,
            config_1.default.routes,
            config_1.default.headers
        );
    }
    parseSearch(query) {
            ...
            try {
                ...
                const aniParsePromises = searchResult.map((item) => {
                    return new Promise((resolve) => {
                        this._getAnimeDetails(item.url).then(
                            (animeDetailsData) => {
                                const animeData =
                                    formatter_1.default.formatAnimeData(
                                        animeDetailsData
                                    );
                                if (
                                    !animeData.sourcePlayer.includes(
                                        "/engine/ajax"
                                    )
                                )
                                    return resolve(
                                ...
                                this.parsePlayer(
                                    animeData.sourcePlayer,
                                    item.url
                                ).then((iframeUrl) => {
                                    const fullAnimeData = Object.assign(
                                        Object.assign(
                                            Object.assign({}, animeData),
                                            item
                                        ),
